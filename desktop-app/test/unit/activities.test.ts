...
describe("Activity Sampling", () => {
  ...
  describe("Query recent activities", () => {
    ...
    it("should returns an empty result when no activities are logged", async () => {
      const eventStore = EventStore.createNull({
        events: [[]],
      });
      const service = ActivitiesService.createNull({ eventStore });
  ...
  describe("Query report", () => {
    it("should summarize hours worked for clients", async () => {
      const eventStore = EventStore.createNull({
        events: [
          [
            ActivityLoggedEventDto.createTestData({
    ...
    it.todo(
      "Summarize hours worked on projects and combines projects with multiple clients",
    );
